...
public class StaffManageServiceImpl implements StaffManageService {
    ...
    @Override
    public ServiceResult<List<StaffVO>> getUserList(String suiteKey, String corpId, Long department_id,Long offset,Integer size,String order){
    	try{
        	ServiceResult<CorpTokenVO> corpTokenSr = corpManageService.getCorpToken(suiteKey, corpId);
            String corpToken = corpTokenSr.getResult().getCorpToken();
            CorpUserDetailList cudlist = corpUserService.getCorpUserList(corpToken,department_id,offset,size,order);
            List<StaffVO> ls = new ArrayList<StaffVO>();
            for(CorpUserDetail corpUserDetail:cudlist.getUserlist()){
            	StaffVO staffVO = StaffConverter.corpUser2StaffVO(corpUserDetail, corpId);
            	ls.add(staffVO);
            }
            return ServiceResult.success(ls);
    	}
    	catch(Exception e){
