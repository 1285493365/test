...
public class DeptManageServiceImpl implements DeptManageService {
    ...
    public ServiceResult<List<Department>> getDeptList(Long deptId, String corpId, String suiteKey){
    	try{
    		ServiceResult<CorpTokenVO> corpTokenSr = corpManageService.getCorpToken(suiteKey, corpId);
    		List<Department> departmentList = corpDepartmentService.getDeptList(corpTokenSr.getResult().getCorpToken(), deptId.toString());
    		
            return ServiceResult.success(departmentList);
    	}
    	catch(Exception e){
